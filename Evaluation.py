...
import dat.file_pathes as dat
if sys.version_info[0] == 2:
    from evaluationpy2.evaluate_tagging_result import get_val_result
    from evaluationpy2.tagging_util import get_label
    from evaluationpy2.tagging_evaluation_util import get_tagging_results
    from evaluationpy2.evaluate_voting_result import get_voting_val_result, get_voting_val_result_in_ans
    from evaluationpy2.evaluation_stats_util import F1Stats
else:
...
class Options:
    def __init__(self, test_or_valid, fuzzy):
        self.test_file = dat.web_qa_valid_dat if test_or_valid == 'valid' else dat.web_qa_test_dat
        self.schema = 'BIO2'
        self.fuzzy = fuzzy
...
def eval_it(p_lbs, valid_or_test='test'):
    options = Options(valid_or_test, fuzzy=False)
